...
# export AFL_LOG_FILE="qemu_log.txt"
# gdbserver 127.0.0.1:1234 \
../afl-fuzz -i ./inputs -o ./outputs -P HTTP -N tcp://127.0.0.1/18045 -m 4096M -QQ \
     -d -q 3 -s 3 -R -W 5 -w 70000 -t 700000\
     -- \
    ...
    -net nic \
    -net user,hostfwd=:127.0.0.1:18045-:8045,hostfwd=:127.0.0.1:14817-:4817,hostname=qemu \
    -d plugin,nochain \
